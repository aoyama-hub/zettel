...
- `archived: true` hides a note from Unprocessed. For fleeting notes it's set by the 7-day cleanup (with `archived_at`); for literature notes it means "already made permanent".
...
- **Lists**: three tabs: **Fleeting**, **Permanent**, and **References**. **Archive** and **Settings** are at the bottom. The search bar matches titles and note text; for a match in the text, the result line shows the matching passage. Permanent notes and references can be ordered **A–Z** or **Recent** (by last edit; saving a permanent note records `updated_at`).
- **Fleeting tab**: fleeting notes grouped under small date labels by last edit, newest first. Tap one to turn it into a permanent note: it opens prefilled with the fleeting text. Give it a title and rewrite it. When you tap **Done** or leave the note, and it has saved with a title, the fleeting original is deleted. A literature original is never deleted: it's marked processed and stays in References.
- **Export**: on the Permanent tab, **Export** downloads one `.txt` with every permanent note in the current order. Each note starts with a separator, then `TITLE:` and `CREATED:`, followed by its text. On iPhone it opens the share sheet, so you can save to Files. The per-note Export in the editor still copies a note and its linked notes to the clipboard.
...
- **References**: literature notes grouped by their source, which can be any free text: a book, article, interview, post, or URL. Open a reference to read its notes in order, add new ones, and see the permanent notes that cite it through a `Source:` line. The capture screen's Source field suggests references you've used before, so notes on the same source stay together.
