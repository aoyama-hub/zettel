...
- **Processing**: new captures appear under *Unprocessed* at the bottom of the Notes tab. Tap one to start a permanent note prefilled with its text, give it a title, and rewrite it. When you tap **Done** or leave the note, and it has saved with a title, the original is handled automatically. A **fleeting** original is deleted. A **literature** original is marked processed and stays in References.
- **Fleeting lifecycle**: a background cleanup runs at most every 6 hours and makes a single commit. Fleeting notes older than 7 days are marked `archived: true` and listed under the **Archive** tab, where you can still make them permanent. Fleeting notes unchanged for 90 days are deleted. Before a deletion, the file's git history is checked, and an edit made elsewhere (for example in Obsidian) is recorded as `updated_at`, which resets the clock. Only files in `fleeting/` tagged `type: fleeting` are ever considered.
