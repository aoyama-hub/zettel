...
- **Export**: on the Permanent tab, **Export** downloads one `.txt` with every permanent note in the current order. Each note starts with a separator, then `TITLE:` and `CREATED:`, followed by its text. On iPhone it opens the share sheet, so you can save to Files.
...
- **References**: literature notes grouped by their source, which can be any free text: a book, article, interview, post, or URL. Open a reference to read its notes in order, add or delete notes, and see the permanent notes that cite it through a `Source:` line. The capture screen's Source field suggests references you've used before, so notes on the same source stay together.
