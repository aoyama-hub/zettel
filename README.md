...
- **Export**: on the Permanent tab, **Export** downloads one `.txt` with every permanent note in the current order. It opens with a short prompt telling the AI what the notes are (your own distilled ideas, not quotes), how the format works, and how to use them. Formatting is kept minimal to save tokens: each note follows a bare `---` line and is its title, a line with its creation date (and references, if any), then its text. Divider lines (`---`) inside a note are exported as `***` so they can't be mistaken for the separator. On iPhone it opens the share sheet, so you can save to Files.
