...
- **Export**: on the Permanent tab, **Export** downloads one `.txt` with every permanent note in the current order. Formatting is kept minimal to save LLM tokens: each note is its title, a line with its creation date (and references, if any), then its text, with a bare `---` line between notes. On iPhone it opens the share sheet, so you can save to Files.
